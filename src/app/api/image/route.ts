...
export async function POST(req: Request) {
  ...
  // 并发提交�?prompt 时，上游（Nano Banana Pro 等）可能会因请求 payload
  // 完全一致而返回同一张缓存图。这里如果调用方没传 seed，就兜底补一�?  // 随机 seed，让每次请求�?body 不同�?  const rawParams: Record<string, unknown> = { ...(rawParamsIn || {}) };
  ...
  if (!model || model.type !== "image") return NextResponse.json({ error: "模型不可�? }, { status: 400 });
  ...
  // 加载 optionPrices —�?需要用它算选项级价�?  const primaryWithOverrides = channel
    ? await prisma.channel.findUnique({ where: { id: channel.id }, include: { optionPrices: true } })
    : null;

  // 估价�?primary �?override 价（悲观：若失败降级，再按真正命中的渠道算）
  ...
  // 本地 /uploads URL 上游访问不到；这里自动改写为 base64 data URL 再下�?  const rewrittenParams = await rewriteLocalRefsToBase64(rawParams);
    ...
    const hint =
      /参数格式错误|格式不对|invalid image|unreachable/i.test(raw)
        ...
        : /context deadline exceeded|client\.timeout|timeout|timed out|etimedout|econnreset/i.test(raw)
          ? "（上游当前繁�?超时，系统已自动重试；仍失败可改用自动渠道或稍后重试�?
        : "";
  ...
  // 真正服务我们的那条渠道（降级后可能和 primary 不同�?  const servedChannelId =
    typeof result.meta?.served_channel_id === "string"
      ? (result.meta.served_channel_id as string)
      : channel?.id ?? null;

  // 按实际命中的渠道重新算一次选项价（不同渠道�?4K 定价可能不同�?  let effective = estimatedEffective;
